...
public final class WebAgentService {
    ...
    public WebAgentService(ToolExecutor executor){this(executor,null,null);}

    /** Constructor with persistence but no orchestrator (M0.3 compat). */
    public WebAgentService(ToolExecutor executor, AssistantPorts.Repository assistantRepo){this(executor,assistantRepo,null);}
    ...
    public AgentResponse handle(AgentRequest request){
        ...
        if(request.turnId()!=null){
            ...
            turnId=assistantRepo.appendTurn(sessionId,Role.USER,request.message(),contextJson);
        }else{
            turnId=UUID.randomUUID();
        }

        // Check injection
        if(isInjection(request.message())){
        ...
        if(orchestrator!=null&&sessionId!=null){
            OrchestrationResult or = orchestrator.orchestrate(
                    turnId, sessionId, request.message(), request.context(), request.confirmed());
            List<String> tools = or.executions().stream().map(e->e.tool()).distinct().toList();
            if(or.confirmationRequired()){
                return new AgentResponse(or.summary(),List.of(or.confirmingTool()),Map.of(),true,turnId,sessionId);
            }
        ...
        String tool=plan(request.message());
        if(tool==null){
        ...
        Map<String,Object> result=executor.execute(tool,request.message(),request.context());
