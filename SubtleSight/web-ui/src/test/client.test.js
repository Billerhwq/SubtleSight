...
describe('api client', () => {
    ...
    it('returns json and sends csrf on writes', async () => { Object.defineProperty(document, 'cookie', { writable: true, value: 'XSRF-TOKEN=abc' }); const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ ok: true }), { status: 200, headers: { 'content-type': 'application/json' } })); await expect(post('/x', { value: 1 })).resolves.toEqual({ ok: true }); expect(new Headers(fetchMock.mock.calls[0][1]?.headers).get('X-XSRF-TOKEN')).toBe('abc'); expect(new Headers(fetchMock.mock.calls[0][1]?.headers).get('Content-Type')).toBe('application/json'); });
    it('supports get delete text and no-content responses', async () => { const fetchMock = vi.spyOn(globalThis, 'fetch'); fetchMock.mockResolvedValueOnce(new Response('hello', { status: 200, headers: { 'content-type': 'text/plain' } })); await expect(get('/text')).resolves.toBe('hello'); fetchMock.mockResolvedValueOnce(new Response(null, { status: 204 })); await expect(del('/item')).resolves.toBeUndefined(); });
    it('surfaces problem detail', async () => { vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ detail: 'bad input' }), { status: 400, headers: { 'content-type': 'application/problem+json' } })); await expect(api('/x')).rejects.toEqual(expect.objectContaining({ status: 400, detail: 'bad input' })); });
    it('falls back to status text for non-json errors', async () => { vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response('down', { status: 503, statusText: 'Unavailable' })); await expect(api('/x')).rejects.toEqual(expect.objectContaining({ status: 503, detail: 'Unavailable' })); });
    describe('knowledge upload', () => {
        ...
        it('uploads with csrf header and progress events', async () => {
            ...
            class FakeXHR {
                ...
                open = vi.fn();
                setRequestHeader = (k, v) => { sent.push(k + '=' + v); };
                send = (_body) => { this.upload.onprogress?.({ lengthComputable: true, loaded: 50, total: 100 }); this.onload?.(); };
            }
